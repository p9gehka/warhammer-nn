...
import { Channel0Name, Channel1Name, Channel2Name, Channel3Name } from '../../environment/nn-input.js';
...
export class MoveAgent extends MoveAgentBase {
	...
	playStepByInput(input) {
		...
		if (input[Channel1Name.Stamina0].some(pos => eq(pos, selected))) {
			...
			let vector = [100,100];
			const deltasDistance = Object.values(Channel2Name).map(objectaName => {	
				if(input[objectaName].length === 0) {
					return 100000;
				}
				const objectCenter = add(input[objectaName][0], [3, 3]);
				const distances = Object.values(Channel0Name).filter(modelName => input[modelName][0] !== undefined).map(modelName => {
					return len(mul(sub(input[modelName][0], objectCenter), -1));
				});
				return Math.max(...distances) ** 2 - Math.min(...distances);
			})


			const objectName = Object.values(Channel2Name)[argMin(deltasDistance)];	
			const objectRandomPoint = input[objectName][getRandomInteger(0, input[objectName].length)]


			vector = mul(sub(selected, objectRandomPoint), -1);
			let [x, y] = vector;
