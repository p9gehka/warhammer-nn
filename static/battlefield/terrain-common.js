	...
	isVisible(point1, point2) {
		...
		for (let footprintEdge of footprints) {
			...
			for (let i = 0; i < footprintEdge.length; i++) {
				const intersection = getLineIntersection(visibilityLine, [footprintEdge[i], footprintEdge[(i+1) % footprintEdge.length]]);
				if (intersection !== null && !eq(intersection, point1) && !eq(intersection, point2)) {
					intersections++;
				}
				if (intersections >= 2) {
					return false;
