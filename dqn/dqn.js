...
export function createDeepQNetwork(numActions, h, w, c, { addSoftmaxLayer } = { addSoftmaxLayer: false}) {
	...
	let conv2d = tf.layers.conv2d({ filters: 16, kernelSize: 6, activation: 'relu'}).apply(inputConv2d);
	conv2d = tf.layers.batchNormalization().apply(conv2d);
	conv2d = tf.layers.conv2d({ filters: 32, kernelSize: 4, activation: 'relu' }).apply(conv2d);
	conv2d = tf.layers.batchNormalization().apply(conv2d);
	conv2d = tf.layers.conv2d({ filters: 32, kernelSize: 4, activation: 'relu'}).apply(conv2d);
