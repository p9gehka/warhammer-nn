...
export function createDeepQNetwork(numActions, h, w, c, hyperParams = defaultHyperparams) {
	...
	model.add(tf.layers.conv2d({filters: 16, kernelSize: [6,4], activation: 'relu', inputShape, ...hyperParams[0] }));
	model.add(tf.layers.batchNormalization());
	model.add(tf.layers.conv2d({filters: 32, kernelSize: [4,3], activation: 'relu', ...hyperParams[1] }));
	model.add(tf.layers.batchNormalization());
