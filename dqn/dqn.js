...
export function createDeepQNetwork(numActions, h, w, c, { addSoftmaxLayer } = { addSoftmaxLayer: false}) {
	...
	let conv2d = tf.layers.conv2d({ filters: 8, kernelSize: 4, activation: 'gelu'}).apply(inputConv2d);
	conv2d = tf.layers.conv2d({ filters: 16, kernelSize: 4, activation: 'gelu' }).apply(conv2d);
	conv2d = tf.layers.conv2d({ filters: 16, kernelSize: 4, activation: 'gelu' }).apply(conv2d);
	conv2d = tf.layers.conv2d({ filters: 16, kernelSize: 4, activation: 'gelu' }).apply(conv2d);
