...
export function createDeepQNetwork(numActions, h, w, c) {
	...
	model.add(tf.layers.conv2d({filters: 2, kernelSize: 8, activation: 'relu', inputShape }));
	model.add(tf.layers.batchNormalization());
	model.add(tf.layers.conv2d({filters: 4, kernelSize: 4, activation: 'relu'}));
	model.add(tf.layers.batchNormalization());
	model.add(tf.layers.conv2d({filters: 4, kernelSize: 3, activation: 'relu' }));
